<template>
    <div>
        ...
        <v-dialog v-model="dialog" max-width="500px">
            ...
            <v-card>
                <form @submit.prevent="store">
                    <v-card-title>
                        <span class="headline">Naam {{ this.itemEdit !== null ? 'bewerken' : 'toevoegen' }}</span>
                    </v-card-title>
                    ...
                    <v-card-actions>
                        ...
                        <v-btn color="primary" flat type="submit">Naam {{ this.itemEdit !== null ? 'opslaan' : 'toevoegen' }}</v-btn>
                    </v-card-actions>
                </form>
            </v-card>
        </v-dialog>
        ...
        <v-dialog v-model="Object.keys( deleteItem ).length > 1" style="max-width: 400px">
            <v-card>
                <v-card-title>
                    <span class="headline">Naam verwijderen</span>
                </v-card-title>

                <v-card-text>
                    Weet je zeker dat je het volgende naam wil verwijderen: <strong>{{ deleteItem.name }}</strong>?
                </v-card-text>
...
<script>
	...
	export default
	{
		...
		computed: {
			 ...
			 *
             *
             * {nameIndex|default.mutations.nameIndex|default.actions.nameIndex|default.getters.nameIndex}
			 */
			items()
			{
			 ...
			 * @returns {default.getters.nameTotal|nameTotal}
			 */
			totalItems()
			{
				return this.$store.getters.nameTotal;
			},
		...
		methods: {
			...
			{
				this.loading = true;
				this.$store.dispatch( 'nameIndex', this.pagination ).then( () => {
					this.loading = false;
				});
			},
			...
			store()
			{
				...
				this.$store.dispatch( this.itemEdit !== null ? 'nameUpdate' : 'nameStore', this.form ).then( () => {
			...
			{
				this.loading = true;
				this.$store.dispatch( 'nameDestroy', id ).then( () =>
				{
					this.data(); // Refresh data
					this.deleteItem = {};
				});
