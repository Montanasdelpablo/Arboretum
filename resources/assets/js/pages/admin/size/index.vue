<template>
    <div>
        ...
        <v-dialog v-model="dialog" max-width="500px">
            ...
            <v-card>
                <form @submit.prevent="store">
                    <v-card-title>
                        <span class="headline">Grootte {{ this.itemEdit !== null ? 'bewerken' : 'toevoegen' }}</span>
                    </v-card-title>
                    ...
                    <v-card-actions>
                        ...
                        <v-btn color="primary" flat type="submit">Grootte {{ this.itemEdit !== null ? 'opslaan' : 'toevoegen' }}</v-btn>
                    </v-card-actions>
                </form>
            </v-card>
        </v-dialog>
        ...
        <v-dialog v-model="Object.keys( deleteItem ).length > 1" style="max-width: 400px">
            <v-card>
                <v-card-title>
                    <span class="headline">Grootte verwijderen</span>
                </v-card-title>

                <v-card-text>
                    Weet je zeker dat je de volgende grootte wil verwijderen: <strong>{{ deleteItem.name }}</strong>?
                </v-card-text>
...
<script>
	...
	export default
	{
		...
		computed: {
			 ...
			 * Get all items
			 *
			 * @returns {sizeIndex|default.mutations.sizeIndex|default.actions.sizeIndex|default.getters.sizeIndex}
			 */
			items()
			{
			 ...
			 * @returns {default.getters.sizeTotal|sizeTotal}
			 */
			totalItems()
			{
				return this.$store.getters.sizeTotal;
			},
		...
		methods: {
			...
			{
				this.loading = true;
				this.$store.dispatch( 'sizeIndex', this.pagination ).then( () => {
					this.loading = false;
				});
			},
			...
			store()
			{
				...
				this.$store.dispatch( this.itemEdit !== null ? 'sizeUpdate' : 'sizeStore', this.form ).then( () => {
			...
			{
				this.loading = true;
				this.$store.dispatch( 'sizeDestroy', id ).then( () =>
				{
					this.data(); // Refresh data
					this.deleteItem = {};
				});
