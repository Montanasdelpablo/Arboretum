<template>
    <div>
        ...
        <v-dialog v-model="dialog" max-width="500px">
            ...
            <v-card>
                <form @submit.prevent="store">
                    <v-card-title>
                        <span class="headline">Belang {{ this.itemEdit !== null ? 'bewerken' : 'toevoegen' }}</span>
                    </v-card-title>
        ...
        <v-dialog v-model="Object.keys( deleteItem ).length > 1" style="max-width: 400px">
            <v-card>
                <v-card-title>
                    <span class="headline">Belang verwijderen</span>
                </v-card-title>

                <v-card-text>
                    Weet je zeker dat je het volgende belang wil verwijderen: <strong>{{ deleteItem.name }}</strong>?
                </v-card-text>
...
<script>
	...
	export default
	{
		...
		computed: {
			 ...
			 *
             *
             * {priorityIndex|default.mutations.priorityIndex|default.actions.priorityIndex|default.getters.priorityIndex}
			 */
			items()
			{
			 ...
			 * @returns {default.getters.priorityTotal|priorityTotal}
			 */
			totalItems()
			{
				return this.$store.getters.priorityTotal;
			},
		...
		methods: {
			...
			{
				this.loading = true;
				this.$store.dispatch( 'priorityIndex', this.pagination ).then( () => {
					this.loading = false;
				});
			},
			...
			{
				this.loading = true;
				this.$store.dispatch( 'priorityDestroy', id ).then( () =>
				{
					this.data(); // Refresh data
					this.deleteItem = {};
				});
