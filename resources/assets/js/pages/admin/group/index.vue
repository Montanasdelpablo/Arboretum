<template>
    <div>
        ...
        <v-dialog v-model="dialog" max-width="500px">
            ...
            <v-card>
                <form @submit.prevent="store">
                    <v-card-title>
                        <span class="headline">Groep {{ this.itemEdit !== null ? 'bewerken' : 'toevoegen' }}</span>
                    </v-card-title>
                    ...
                    <v-card-actions>
                        ...
                        <v-btn color="primary" flat type="submit">Groep {{ this.itemEdit !== null ? 'opslaan' : 'toevoegen' }}</v-btn>
                    </v-card-actions>
                </form>
            </v-card>
        </v-dialog>
        ...
        <v-dialog v-model="Object.keys( deleteItem ).length > 1" style="max-width: 400px">
            <v-card>
                <v-card-title>
                    <span class="headline">Groep verwijderen</span>
                </v-card-title>

                <v-card-text>
                    Weet je zeker dat je de volgende groep wil verwijderen: <strong>{{ deleteItem.name }}</strong>?
                </v-card-text>
...
<script>
	...
	export default
	{
		...
		computed: {
			 ...
			 *
			 * @returns
			 * {groupIndex|default.mutations.groupIndex|default.actions.groupIndex|default.getters.groupIndex}
			 */
			items()
			{
			 ...
			 * @returns {default.getters.groupTotal|groupTotal}
			 */
			totalItems()
			{
		...
		methods: {
			...
			store()
			{
				...
				this.$store.dispatch( this.itemEdit !== null ? 'groupUpdate' : 'groupStore', this.form ).then(
					() =>
					{
