<template>
    <div>
        ...
        <v-dialog v-model="dialog" max-width="500px">
            ...
            <v-card>
                <form @submit.prevent="store">
                    <v-card-title>
                        <span class="headline">Soort {{ this.itemEdit !== null ? 'bewerken' : 'toevoegen' }}</span>
                    </v-card-title>
                    ...
                    <v-card-actions>
                        ...
                        <v-btn color="primary" flat type="submit">Soort {{ this.itemEdit !== null ? 'opslaan' : 'toevoegen' }}</v-btn>
                    </v-card-actions>
                </form>
            </v-card>
        </v-dialog>
        ...
        <v-dialog v-model="Object.keys( deleteItem ).length > 1" style="max-width: 400px">
            <v-card>
                <v-card-title>
                    <span class="headline">Soort verwijderen</span>
                </v-card-title>

                <v-card-text>
                    Weet je zeker dat je de volgende soort wil verwijderen: <strong>{{ deleteItem.name }}</strong>?
                </v-card-text>
...
<script>
	...
	export default
	{
		...
		computed: {
			 ...
			 *
			 * @returns
			 * {specieIndex|default.mutations.specieIndex|default.actions.specieIndex|default.getters.specieIndex}
			 */
			items()
			{
			 ...
			 * @returns {default.getters.specieTotal|specieTotal}
			 */
			totalItems()
			{
				return this.$store.getters.specieTotal;
			},
		...
		methods: {
			...
			{
				this.loading = true;
				this.$store.dispatch( 'specieIndex', this.pagination ).then( () => {
					this.loading = false;
				});
			},
			...
			store()
			{
				...
				this.$store.dispatch( this.itemEdit !== null ? 'specieUpdate' : 'specieStore', this.form ).then( () => {
			...
			{
				this.loading = true;
				this.$store.dispatch( 'specieDestroy', id ).then( () =>
				{
					this.data(); // Refresh data
					this.deleteItem = {};
				});
